modded class RespawnDialogue
{
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets("DeutschZ_MenuMusic_V2_Test/gui/dzkothg_respawn.layout");
		if (!layoutRoot)
			return super.Init();
	...
	override bool OnMouseEnter(Widget w, int x, int y)
	{
		...
		if (id == DZSPAWN_ID_RANDOM || id == DZSPAWN_ID_COAST || id == DZSPAWN_ID_SAFE || id == DZSPAWN_ID_LOADOUT || id == ID_RESPAWN_CUSTOM)
			DZSPAWN_SetButtonImage(w, "DeutschZ_MenuMusic_V2_Test/gui/darkglass_v1/02_buttons_text/button_wide_empty_hover.paa");
		else if (id == IDC_CANCEL)
			DZSPAWN_SetButtonImage(w, "DeutschZ_MenuMusic_V2_Test/gui/darkglass_v1/02_buttons_text/button_small_empty_hover.paa");
		return true;
	}
	...
	override bool OnMouseLeave(Widget w, Widget enterW, int x, int y)
	{
		...
		int id = w.GetUserID();
		if (id == DZSPAWN_ID_RANDOM || id == DZSPAWN_ID_COAST || id == DZSPAWN_ID_SAFE || id == DZSPAWN_ID_LOADOUT || id == ID_RESPAWN_CUSTOM)
			DZSPAWN_SetButtonImage(w, "DeutschZ_MenuMusic_V2_Test/gui/darkglass_v1/02_buttons_text/button_wide_empty_normal.paa");
		else if (id == IDC_CANCEL)
			DZSPAWN_SetButtonImage(w, "DeutschZ_MenuMusic_V2_Test/gui/darkglass_v1/02_buttons_text/button_small_empty_normal.paa");
		return true;
	}
